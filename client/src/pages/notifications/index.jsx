...
import { Box, useTheme, Grid, Button, Toolbar, Typography } from "@mui/material";
...
const Notifications = ({ changeWelcomeText }) => {
  ...
  const handleApprove = async () => {
    ...
    const updatePromises = selectedRows.map(async (row) => {
      ...
      const response = await fetch(`http://127.0.0.1:5000/auth/api/users/notifications/${row.id}`, {
        method: 'PUT',
        headers: {
    ...
    Promise.all(updatePromises).then((updatedRows) => {
      setNotifications((prevNotifications) =>
        prevNotifications.map((notification) => {
          const updatedRow = updatedRows.find((row) => row && row.id === notification.id);
      ...
      const createIncidentPromises = selectedRows.map(async (row) => {
        ...
        const postResponse = await fetch('http://127.0.0.1:5000/auth/api/users/incidents', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      console.error('Error with updating notifications or creating incidents:', error);
    });
  };
  ...
  const handleDenyClick = async () => {
    ...
    const updatePromises = selectedRows.map(async (row) => {
      ...
      return response.ok ? { ...row, status: updatedStatus, type: updatedType } : null;
    });
  ...
  const handleIgnoreClick = async () => {
    ...
    // Execute all the PUT requests and update the state once all are done
    Promise.all(updatePromises).then((updatedRowIds) => {
      setNotifications((prevNotifications) =>
        prevNotifications.filter(notification => 
          !updatedRowIds.includes(notification.id)  // Remove ignored notifications from state
        )
      );
    }).catch((error) => {
      console.error('Error updating notifications:', error);
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await fetch('http://127.0.0.1:5000/auth/api/users/notifications');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
...
}, [changeWelcomeText]); // Dependencies array
